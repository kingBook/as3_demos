	...
	public function Initialize(step:b2TimeStep, contacts:Vector.<b2Contact>, contactCount:int, allocator:*):void
	{
		...
		for (i = 0; i < contactCount; ++i)
		{
			...
			if(!bodyA.m_allowBevelSlither||bodyA.m_uphillZeroFriction){
				...
				if((planeAngle>bevel&&planeAngle<90-bevel) || (planeAngle>180+bevel&&planeAngle<270-bevel)){//斜面 左上角-右下角
				...
				}else if((planeAngle>90&&planeAngle<180-bevel) || (planeAngle>270+bevel&&planeAngle<360-bevel)){//斜面 左下角-右上角
					if(!bodyA.m_allowBevelSlither){
						if(vx<=0)cc.friction=1;
					}
					if(bodyA.m_uphillZeroFriction){
