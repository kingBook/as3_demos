...
public class b2AABB
{
	...
	*/
	...
	public function toString(ptm_ratio:Number=1):String{
		return "lowerBound:"+lowerBound.toString(ptm_ratio)+" upperBound:"+upperBound.toString(ptm_ratio);
	}
	
	public function ToRect(ptm_ratio:Number=1):Rectangle{
		var x:Number=lowerBound.x*ptm_ratio;
		var y:Number=lowerBound.y*ptm_ratio;
		var w:Number=(upperBound.x-lowerBound.x)*ptm_ratio;
		var h:Number=(upperBound.y-lowerBound.y)*ptm_ratio;
		return new Rectangle(x,y,w,h);
	}
	
	public function ContainsV(v:b2Vec2):Boolean{
		var result:Boolean=true;
		result&&=lowerBound.x<v.x;
		result&&=lowerBound.y<v.y;
		result&&=upperBound.x>v.x;
		result&&=upperBound.y>v.y;
		return result;
	}
	
	static public function Make(lowerBound:b2Vec2,upperBound:b2Vec2):b2AABB {
		var aabb:b2AABB=new b2AABB();
	...
	static public function MakeWH(w:Number,h:Number,centerX:Number,centerY:Number):b2AABB{
		var aabb:b2AABB=new b2AABB();
		var wf:Number=w*0.5;
		var hf:Number=h*0.5;
